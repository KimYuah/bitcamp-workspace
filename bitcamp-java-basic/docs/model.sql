...
-- 학력
DROP TABLE IF EXISTS lms_adu_level RESTRICT;
...
-- 학력
ALTER TABLE lms_adu_level
  ADD CONSTRAINT PK_lms_adu_level -- 학력 기본키
    PRIMARY KEY (
      elno -- 학력번호
    );

-- 학력 인덱스
CREATE INDEX IX_lms_adu_level
  ON lms_adu_level( -- 학력
    title ASC -- 학력명
  );
...
-- 학생
ALTER TABLE lms_stud
  ADD CONSTRAINT FK_lms_adu_level_TO_lms_stud -- 학력 -> 학생
    FOREIGN KEY (
      elno -- 학력번호
    )
    REFERENCES lms_adu_level ( -- 학력
      elno -- 학력번호
    );
...
-- 강사
ALTER TABLE lms_tchr
  ADD CONSTRAINT FK_lms_adu_level_TO_lms_tchr -- 학력 -> 강사
    FOREIGN KEY (
      elno -- 학력번호
    )
    REFERENCES lms_adu_level ( -- 학력
