...
public class DefaultProjectService implements ProjectService { // BusinessLogic, 업무관련
  ...
  public int delete(int no) throws Exception {
    try {
      factoryProxy.statrtTransaction();

      taskDao.deleteByProjectNo(no);
      projectDao.deleteMembers(no);
      //if (100 == 100) throw new Exception("일부러예외발생");
  ...
  @Override
    public List<Project> list(String keyword) throws Exception {
      ...
      // => 프로그래밍의 일관성을 위해서다.
      // => 커맨드 객체가 상황에 따라 Service 객체를 쓰거나 Dao 객체를 써야한다면
      //    프로그래밍의 일관성이 없어서 유지보수하기가 어렵다.
      // => 커맨드 객체가 서비스 객체를 사용하기로 했으면
      //    어떤 작업을 수행하든지 간에 일관되게 서비스 객체를 사용하는 것이
      //    유지보수하기에 더 낫다.
      // => 그래서 이런 메서드를 만드는 것이다.//
